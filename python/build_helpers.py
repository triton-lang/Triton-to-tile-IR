...
def get_nvidia_toolchain_packages():
    ...
    return [
        ...
        # Bundle the released TileIR runtime independently of the PTX backend.
        NvidiaToolchainPackage(
            name="tileiras",
            component="cuda_tileiras",
            version=versions["tileiras"],
            src_path=f"bin/tileiras{exe}",
            dst_path="tileir_cuda/bin/tileiras",
            override_attr="tileiras_path",
        ),
        NvidiaToolchainPackage(
            name="tileir-ptxas",
            component="cuda_nvcc",
            version=versions["tileir-ptxas"],
            src_path=f"bin/ptxas{exe}",
        ...
        NvidiaToolchainPackage(
            name="tileir-libnvvm",
            component="libnvvm",
            version=versions["tileir-libnvvm"],
            src_path="nvvm/lib64",
...
def download_and_copy_dependencies(helper_args: BuildHelperArgs):
    ...
    for package in get_nvidia_toolchain_packages():
        download_and_copy(
            ...
            override_path=getattr(helper_args, package.override_attr) if package.override_attr else None,
