...
from triton.backends.cutile.errors import HitFallback
from triton.runtime.cache import get_cache_manager
from triton.backends.compiler import BaseBackend, GPUTarget, Language
from triton.backends.cutile.conf import CuTileEnvConf
from triton._C.libtriton import ir, passes, cutile
...
class CuTileOptions:
    ########################## cuTile core options ##########################
    backend_name: str = 'cutile'
    arch: str = None
    num_ctas: int = 1
    # cutile use num_stages to control the op cost, see <cutile_link>
    num_stages: int = 3 
    # cutile use opt_level to control the optimization level, see <cutile_link>
    opt_level: int = 3
    # cutile use occupancy to control the register usage, see <cutile_link>
    occupancy: int = 1
    # cutile use enable_fp_fusion to control the fma fusion, see <cutile_link>
    enable_fp_fusion: bool = True
    cutile_tileiras_path: str = CuTileEnvConf.get_tileiras_path()
    ...
    # cutile doesn't need these flags, just for compatibility with other backend
    num_warps: int = 4
    ...
    # maxnreg in cutile backend is just for compatibility with other backend
    # cutile use occupancy to control the register usage.
    ...
    @property
    def enable_ftz(self):
        return CuTileEnvConf.enable_ftz()

    @property
    def enable_approx(self):
        return CuTileEnvConf.enable_approx()
...
class CuTileBackend(BaseBackend):
    ...
    def parse_options(self, opts) -> Any:
        ...
        args.update(
            {
                k: opts[k]
                for k in CuTileOptions.__dataclass_fields__.keys()
                if k in opts
        ...
        if "supported_fp8_dtypes" not in args:
            supported_fp8_dtypes = set(CuTileOptions.supported_fp8_dtypes)
    ...
    def load_dialects(self, ctx):
        cutile.load_dialects(ctx)

    @staticmethod
    def call_tileiras(mod, metadata, opt: CuTileOptions, capability):
        tileiras = opt.cutile_tileiras_path
        tileiras_cmd = [
            tileiras,
        ...
        with tempfile.NamedTemporaryFile(delete=False, mode='r', suffix='.log') as flog, \
            ...
            bytecode = cutile.write_bytecode(mod)
    ...
    @staticmethod
    def make_ttir(mod, metadata, opt: CuTileOptions, capability):
    ...
    @staticmethod
    def make_cutile(mod, metadata, opt: CuTileOptions, capability):
        pm = ir.pass_manager(mod.context)
        pm.enable_debug()
        # Inherit LiftControlflowToSCF from upstream to adapt to `ControlFlow` within `triton.func`
        cutile.passes.add_lift_tt_cf_to_scf(pm)
        # The root IR for ttir is builtin moduleOp and all
        # cuda-tile ir must under cutile_moduleOp.
        # So, we will insert an cutile moduleOp directly at the beginning of TritonToCudaTile pass.
        cutile.passes.add_assume_to_cutile(pm)
        cutile.passes.add_triton_to_cudatile(
            pm,
            opt.enable_approx,
            opt.enable_ftz,
            capability,
        ...
        cutile.passes.add_auto_gen_memtoken(
            pm,
            opt.enable_autogen_alias_mem_token
        )
        passes.common.add_inliner(pm)
        if opt.enable_fp_fusion:
            cutile.passes.add_fma_fusion(pm)
        cutile.passes.add_strip_debuginfo(pm)
        pm.run(mod, "make_cutile")
        if not cutile.only_contain_legal_dialects(mod):
            raise RuntimeError(
                "Triton ttir to cutile ir failed. Some ttir ops cannot be converted to cutile."
            )
    ...
    @staticmethod
    def make_cubin(mod, metadata, opt: CuTileOptions, capability):
        return CuTileBackend.call_tileiras(mod, metadata, opt, capability)
    ...
    def add_stages(self, stages, options, language):
        ...
        stages["cuTile"] = lambda src, metadata: self.make_cutile(
            src, metadata, options, capability
        )
    ...
    @functools.lru_cache()
    def hash(self):
        version = get_cutile_version()
        return f"{'cutile'}-{version}-{self.target.arch}"
